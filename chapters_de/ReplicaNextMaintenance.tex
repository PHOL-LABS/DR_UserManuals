...
Der Reiter \emph{Colors} stellt einen segmentbasierten Editor für die WS2812-Hintergrundbeleuchtung bereit. Jede Zeile beschreibt ein Bereichsende, den zugehörigen Funktionsblock sowie den Farbtyp für diesen Bereich. In der Farbtyp-Auswahl verwendet \emph{Main color} den aktuell im Reiter \emph{Control} eingestellten RGB-Wert, \emph{Main color /3} dieselbe Hauptfarbe mit einem Drittel der Intensität (dreimal dunkler), \emph{Custom color} den direkt in diesem Reiter \emph{Colors} ausgewählten RGB-Wert und \emph{Back color} die Hintergrundbeleuchtungsfarbe aus dem Reiter \emph{Control}.
