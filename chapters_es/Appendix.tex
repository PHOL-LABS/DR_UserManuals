...
\begin{table}[htbp]
    ...
    \begin{tblr}{
        ...
        \toprule
        \SetCell{cmd=\relax}\textbf{Comando} & \SetCell{cmd=\relax}\textbf{Nombre} & \textbf{Descripción} \\
        \midrule
...
\begin{table}[htbp]
    ...
    \begin{tblr}{
        ...
        \toprule
        \SetCell{cmd=\relax}\textbf{Parámetro} & \SetCell{cmd=\relax}\textbf{Predeterminado} & \textbf{Notas} \\
        \midrule
...
\chapter{Catálogo de parámetros de \ReplicaNextShort{}}\label{appendix:next-parameters-es}

Todos los parámetros de este catálogo se encuentran en la pestaña \emph{Settings}. Cargue cada parámetro antes de editarlo y cargue y edite únicamente uno a la vez. Los identificadores siguientes son ajustes del firmware; los comandos de parámetros con nombres parecidos de la referencia anterior son interfaces distintas y no deben considerarse el mismo elemento. Las etiquetas y descripciones del firmware se reproducen literalmente para poder identificarlas en el dispositivo.

\def\ParameterCatalogueCaption{Parámetros del firmware en la pestaña \emph{Settings}}
\def\ParameterCatalogueLabel{tbl:next-parameters-es}
\def\ParameterIdentifier{Identificador}
\def\ParameterUILabel{Etiqueta de IU}
\def\ParameterDescription{Descripción}
\def\ParameterValue{Tipo / predeterminado}
\def\ParameterLimits{Límites / notas}
\input{chapters/ParameterCatalogue}
