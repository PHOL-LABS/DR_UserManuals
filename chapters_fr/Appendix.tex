...
\chapter{Catalogue des paramètres \ReplicaNextShort{}}\label{appendix:next-parameters-fr}

Tous les paramètres de ce catalogue se trouvent dans l’onglet \emph{Settings}. Chargez un paramètre avant de le modifier et ne chargez et ne modifiez qu’un seul paramètre à la fois. Les identifiants ci-dessous sont des réglages du micrologiciel ; les commandes de paramètre de nom similaire dans la référence précédente constituent des interfaces distinctes et ne doivent pas être confondues. Les libellés et descriptions du micrologiciel sont reproduits à l’identique afin de pouvoir les retrouver sur l’appareil.

\def\ParameterCatalogueCaption{Paramètres du micrologiciel dans l’onglet \emph{Settings}}
\def\ParameterCatalogueLabel{tbl:next-parameters-fr}
\def\ParameterIdentifier{Identifiant}
\def\ParameterUILabel{Libellé UI}
\def\ParameterDescription{Description}
\def\ParameterValue{Type / défaut}
\def\ParameterLimits{Limites / notes}
