...
\begin{table}[htbp]
    ...
    {\scriptsize
    \begin{tblr}{
        colspec = {Q[c,0.12\linewidth,font=\ttfamily] Q[l,0.32\linewidth,cmd=\seqsplit] Q[l,0.52\linewidth]},
        rowsep = 2pt,
    }
        \toprule
...
\chapter{Каталог параметров \ReplicaNextShort{}}\label{appendix:next-parameters-ru}

Все параметры этого каталога находятся на вкладке \emph{Settings}. Перед изменением параметр необходимо загрузить; загружайте и редактируйте только один параметр за раз. Указанные ниже идентификаторы относятся к настройкам прошивки. Команды параметров с похожими названиями из предыдущего справочника являются отдельным интерфейсом и не должны считаться теми же элементами. Названия и описания из прошивки приведены дословно, чтобы их можно было однозначно найти на устройстве.

\def\ParameterCatalogueCaption{Параметры прошивки на вкладке \emph{Settings}}
\def\ParameterCatalogueLabel{tbl:next-parameters-ru}
\def\ParameterIdentifier{Идентификатор}
\def\ParameterUILabel{Название в UI}
\def\ParameterDescription{Описание}
\def\ParameterValue{Тип / значение}
\def\ParameterLimits{Диапазон / примечания}
