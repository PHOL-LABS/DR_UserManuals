...
The \emph{Colors} tab introduces a segment-based editor for the WS2812 LED backlight. Each row describes a range end point, the functional area it corresponds to, and the colour type used for that range. In the colour type selector, \emph{Main color} uses the RGB value currently set on the \emph{Control} tab, \emph{Main color /3} uses the same main colour at one third of its intensity (three times darker), \emph{Custom color} uses the RGB value selected directly on this \emph{Colors} tab, and \emph{Back color} uses the backlight colour from the \emph{Control} tab.
