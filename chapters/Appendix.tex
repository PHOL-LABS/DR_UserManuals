...
All parameters in this catalogue are located on the \emph{Settings} tab.  Load a
parameter before editing it, and load and edit only one parameter at a time.
The identifiers below are firmware settings; similarly named parameter commands
in the preceding reference are separate interfaces and must not be treated as
the same item.  UI labels and firmware descriptions are reproduced verbatim so
that they can be matched to the device regardless of the manual language.

\def\ParameterCatalogueCaption{\ReplicaNextShort{} firmware parameters in the \emph{Settings} tab}
\def\ParameterCatalogueLabel{tbl:next-parameters}
\def\ParameterIdentifier{Identifier}
\def\ParameterUILabel{UI label}
\def\ParameterDescription{Description}
\def\ParameterValue{Type / default}
\def\ParameterLimits{Limits / notes}
